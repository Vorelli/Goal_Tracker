import { grabFrom, grabAllFrom } from "./grab";
...
TaskAdderOverlayManager.prototype.setValues = function setValues(name, desc, dateS, dateExpected, dateC, completionStatus) {
    let newButton = document.createElement('button');
    this.overlay.querySelector('button').parentNode.replaceChild(newButton, this.overlay.querySelector('button'));

    if(!name) { name = new Task('','','','','',false); }
    if(!!name.name || name.name == '') {//if name is actually a task
        desc = name.desc;
        dateS = !isNaN(Date.parse(name.dateS)) ? _toDate(name.dateS) : "Not Set";
        dateExpected = !isNaN(Date.parse(name.dateExpected)) ? _toDate(name.dateExpected) : "Not Set";
...
TaskAdderOverlayManager.prototype.editButtonText = function editButtonText(newValue) {
...
TaskAdderOverlayManager.prototype.editTask = function editTask(task) {
    ...
    this.toggleOverlay();
    this.overlay.querySelector('button').addEventListener('click', function(task) { this.toggleOverlay(); Listener.trigger('submitEdit', task); }.bind(this, task) )
}
...
TaskAdderOverlayManager.prototype.addNewTaskOverlay = function addNewTaskOverlay() {
    ...
    this.toggleOverlay();
    this.overlay.querySelector('button').addEventListener('click', function() { this.toggleOverlay(); Listener.trigger('submitNewTask'); }.bind(this) )
}
