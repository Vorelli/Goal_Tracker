...
function ViewGenerator() {
    const simplifiedViewRowHTML = "<tr> <td class='name' valign='center'>Go Hard AFgsfadafasdfsadfsadf</td> <td class='dateStart'>10/20/2019</td> <td class='statusTD'> <div class='statusBar'></div> <div class='statusBarLine'></div> </td> <td class='dateComplete'></td> </tr>";
    ...
    function _returnToSimplified(task) { let row = _findRow(task); row.parentNode.replaceChild(_createSimplifiedRow(task), row); };
    ...
    function createTableView(task) {
        let a = new DocumentFragment().appendChild(createElement('body')).appendChild(createElement('div'));
        ...
        for(let i = 0; i < task.subTasks.length; i++) {
            view.querySelector('tbody').appendChild(_createTableRow(task.subTasks[i]));
        }
        Listener.trigger('clearContent');
        grabFrom(a, '#addAmbition').addEventListener('click', tAOM.addNewTaskOverlay.bind(tAOM))
    ...
    function _createTableRow(task) {
        let docFrag = new DocumentFragment();
        docFrag.appendChild(createElement('div')).appendChild(createElement('table')).appendChild(createElement('tbody')).insertAdjacentHTML('beforeEnd', tableViewRowHTML);
        let row = docFrag.querySelector('tr:last-child');
        _fillInView(row, task);
        row.addEventListener('click', function() {let clickedStatusBar = false;event.path.forEach(element => {if(element.classList && element.classList.contains('statusTD')) clickedStatusBar = true;});if(!event.target.classList.contains('goalButton') && !clickedStatusBar)_createDetailedView(task, 6)})
        return  row;
    }
    ...
    function _fillInView(view, task, colSpan) {
        ...
        _tryToSetTextContent(view, '.statusBarLine', (task.progress()*100)+'px', 'style.width')
        _tryToSetTextContent(view, '.statusBarLine', task.onTrack() ? '#474747' : 'red', 'style.backgroundColor')
        for(let j = 0; j < task.subTasks.length; j++)
            grabFrom(view, '.statusBar').appendChild(!!task.subTasks[j].completionStatus ? generateFilledDot() : generateDot());
        commonEvents.forEach( event => {
            let shouldFill = Boolean(event.selector =='.statusBar' && view.querySelector('.dateExpected'));
            let shouldFill2 = Boolean(event.selector =='.statusBarLine' && view.querySelector('.dateExpected'));
            if(event.selector=='button' && colSpan == 6)
                grabFrom(view, event.selector).addEventListener('click', _returnToTableRow.bind(this,task));
            else if(view.querySelector(event.selector) && (shouldFill || shouldFill2 || event.selector!='.statusBarLine' && event.selector!='.statusBar'))
                grabFrom(view, event.selector).addEventListener('click', event.listener.bind(!!event.thisVar ? event.thisVar : this, task));
        })
        return view;
    }

    //attempts to set text content of an element, fearing it could throw an error
    function _tryToSetTextContent(element, elementName, value, whatToSet='textContent') {
        try {
            if(whatToSet.indexOf('style.')!=-1)
                grabFrom(element, elementName)['style'][whatToSet.substring(6,)] = value;
            else
                grabFrom(element, elementName)[whatToSet] = value;
        } catch (error) {
            //console.log(error);
        }
    }
    ...
    function _getTaskUnderParentByName(parentNode, name) {
        ...
        if(!!parentNode) {
            switch(parentNode.type) {
                case'simplified':
                    let tableTitle = grab('#tableTitle');
    ...
    function submitNewTask() {
        ...
        let taskByTitle = grab('#content #tableTitle').colSpan==6 ? _getTaskByName(grab('#content #tableTitle').textContent) : null;
        if(!!taskByTitle){
            let newTask = Listener.trigger('addSubtask', taskByTitle, values.name, values.desc, values.dateS, values.dateC, values.dateExpected, values.completionStatus)
    ...
    function _refreshRow(row, task) {
        ...
        if(row.parentNode.children[0].colSpan == 6)
        ...
        if(colspan == 4) {
            ...
            for(let i = 2; i < tableRows.length; i++) {
                if(!grabFrom(tableRows[i],'.dateExpected'))
                    viewData.rows.push({task: _getTaskByRow(tableRows[i]), type: 'simplified'});
                else
                    viewData.rows.push({task: _getTaskByRow(tableRows[i]), type: 'block'})
            }
            return viewData;
        } else if(colspan == 6) {
            viewData.type = 'table';
